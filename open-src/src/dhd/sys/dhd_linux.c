...
/* eanble HOSIP cache update from the host side when an eth0:N is up */
...
/* IP ADDR  CHG blk notifier fwd decl */
...
// Start Motorola IKNVSSW-1189 Flexible DTIM skipping
// returns number of DTIMs to skip
// if we can not get info from the chip - no DTIM skipping
static int dhd_cfg_dtim_skip(dhd_pub_t *dhd)
{
	int beacon;
	int dtim;
	int ret = 1;
	char iovbuf[32];
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
	// Need to read out current beacon and dtim configuration of the AP
	// we are associated to
	bcm_mkiovar("bi_assoc", 0, 0, iovbuf, sizeof(iovbuf));
	ret = dhdcdc_query_ioctl(dhd, 0, WLC_GET_VAR, iovbuf, sizeof(iovbuf));
	if (ret < 0) {
		DHD_ERROR(("%s: reading beacon duration failed ret %d\n", __FUNCTION__, ret));
		return 1;
	}
	beacon = *(int *)iovbuf;
	bcm_mkiovar("dtim_assoc", 0, 0, iovbuf, sizeof(iovbuf));
	ret = dhdcdc_query_ioctl(dhd, 0, WLC_GET_VAR, iovbuf, sizeof(iovbuf));
	if (ret < 0) {
		DHD_ERROR(("%s: reading dtim duration failed ret %d\n", __FUNCTION__, ret));
		return 1;
	}		
	dtim = *(int*)iovbuf;
	DHD_ERROR(("%s: AP settings: beacon %d dtim %d\n", __FUNCTION__, beacon, dtim));
	// DTIM skipping is pretty risky thing so for now we would
	// do it only on sane configurations of AP
	// that is DTIM 1 and BEACON 100
	if ((100 == beacon) && (1 == dtim))
		return 3;
	DHD_TRACE(("%s: Exit\n", __FUNCTION__));
	return 1;
}
// End Motorola IKNVSSW-1189 Flexible DTIM skipping
		...
		if (value && dhd->in_suspend) {
				...
				*/
				if ((dhd->dtim_skip == 0) || (dhd->dtim_skip == 1))
					bcn_li_dtim = dhd_cfg_dtim_skip(dhd);
				else
...
void aoe_update_host_ipv4_table(dhd_pub_t *dhd_pub, u32 ipa, bool add)
{
	...
	print_buf(ipv4_buf, 32, 4); /* max 8 IPs 4b each */
